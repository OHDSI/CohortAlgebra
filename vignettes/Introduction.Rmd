...
If think of cohort definitions as a logical expression composed of inclusion criteria we might be able to make use of algebraic properties of logical AND and OR.
...
What are the union and intersection operations for circe-be? It might be better to abstact the operations of union and intersection. Namely we have two operations that are each known by several different names:
...
The terms *join*, $\lor$, and *meet*, $\land$, are often used when discussing lattices to we will use these for cohort definitions.


TODO: Define the join (logical OR) and meet (logical AND) operations for Cice-be cohort definitions



# Properties of the generation function $G$

Now that we have a cohort definition lattice and a generated cohort lattice we can explore the properties of the function connecting them, $G$.
...
CohortAlgebra provides the computational tools to explore these properties, combine cohorts and cohort definitions, and built cohort lattices.
